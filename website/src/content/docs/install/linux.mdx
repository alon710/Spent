...
**No menubar on Linux.** The setup script installs the service only. You'll open the dashboard at `http://127.0.0.1:2412` (or `http://spent.local:2412` if you add the hosts entry manually with `sudo`).
</Callout>
