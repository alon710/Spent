...
```sh
git clone https://github.com/alon710/Spent.git ~/Spent
cd ~/Spent
```
...
2. **Adds `127.0.0.1 spent.local` to `/etc/hosts`** so you can reach the dashboard at `http://spent.local:2412`. Asks for `sudo` once.
3. **Registers a LaunchAgent** at `~/Library/LaunchAgents/io.spent.app.plist` that starts the server on login. Server binds to `127.0.0.1:2412` only.
