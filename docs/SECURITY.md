...
Spent is built to run **only on your own machine**. The threat model below assumes you
...
- The encryption key file itself is plaintext (hex). Anyone who can read both
  `data/.encryption-key` AND `data/spent.db` can decrypt your credentials.
...
privileges as the Spent process.
...
In those cases, set `SPENT_DISABLE_CHROMIUM_SANDBOX=1` in your environment. When you can,
...
Spent does not ship a service installer. You run the server yourself with `bun start`,
